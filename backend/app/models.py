...
class User(Base):
    ...
    assigned_tasks = relationship(
        "Task", back_populates="assigned_to_user", foreign_keys="Task.assigned_to"
    )
...
class DocumentChunk(Base):
    """Maps FAISS vector index positions back to source text for retrieval."""
    __tablename__ = "document_chunks"
    id = Column(Integer, primary_key=True, index=True)
    document_id = Column(Integer, ForeignKey("documents.id"), nullable=False)
    chunk_text = Column(Text, nullable=False)
    vector_index = Column(Integer, nullable=False, unique=True)  # position in FAISS index
...
class SearchQuery(Base):
    """Stores every search query for analytics (most searched queries)."""
